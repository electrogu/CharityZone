...
module.exports = {
  ...
  perms: `OWNEROFTHEBOT`,
  run: (message, args, content, client, command1, command2, isCommand) => {
    try {
      ...
      const parsed = mainInfo
        ? parse(Date.now() - mainInfo.date)
        : parse(Date.now());
      let text = mainInfo
        ? `**Date:** <t:${Math.floor(
            mainInfo.unixtimestamp
          )}>\n\`\`\`Time elapsed: ${parsed.hours}:${parsed.minutes}:${
            parsed.seconds
          }\`\`\``
        : `There isn't any maintenance right now, you can use the buttons down below to start or stop a maintenance`;
      ...
      collector.on("end", (ButtonInteraction) => {
        ...
        if (first.customId == `startmt`) {
          db.set(`maintenanceInfo`, {
            date: Date.now(),
            unixtimestamp: Date.now() / 1000,
          });
          first.reply(
            `Maintenance session started: <t:${Math.floor(Date.now() / 1000)}>`
          );
        } else if (first.customId == `stopmt`) {
          db.delete(`maintenanceInfo`);
          first.reply(
            `Maintenance session finished: <t:${Math.floor(Date.now() / 1000)}>`
          );
        }
      });
    } catch (err) {
